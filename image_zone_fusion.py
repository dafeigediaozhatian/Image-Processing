...
def min_distance(pix, pix_list):
    min_dist = 256
    for i in num_list:
        if i != num:
            if min_dist > abs(num - i):
                min_dist = abs(num - i)
...
for i in range(len(pix_counts)):
    #print(df1.index[i])
    #print(pix_list[i])
    if pix_counts[i] < threshold:
        min_dist = min_distance(pix_list[i], pix_list)
        for i in range()
...
for i in dict_rgb:
    #print(dict_rgb[i])
    dict_rgb[i] < 10000:
        #dict_rgb = max(dict_rgb, key=lambda x: )
        dict_rgb = 
